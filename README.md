...
| 项目 | 状态 |
|---|---|
| 当前版本 | v0.1（本地实现完成，尚未发布） |
| 运行平台 | Linux、macOS 13+ |
| 最低依赖 | Git 2.39、Go 1.26（仅构建需要 Go） |
| 运行时依赖 | Go 标准库、系统 Git |
| 许可证 | 尚未确定 |

> 当前没有 canonical repository URL、remote、CI 或发布制品。发布前事项见 [PLAN.md](PLAN.md)。

## 功能

- 用五个命令覆盖常用 worktree 初始化、查看、创建、metadata 编辑和删除。
- 通过 worktree config 保存 `description` 和 `protected`。
...
构建完成后可以直接运行 `bin/gwm`，或将它复制到本机 `PATH` 中的目录。项目尚未发布预编译制品或公开 Go module，当前不要假设安装地址。
...
GWM 只拥有目标工作树中的一个 worktree-scope 配置值：
...
- `description`：可为空，最多 4096 UTF-8 bytes。
- `protected`：为 true 时，`gwm remove` 拒绝删除该工作树。
- Metadata 缺失等价于 `{description:null, protected:false}`。
